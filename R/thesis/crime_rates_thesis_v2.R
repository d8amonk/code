...
fullset_WC <- read.csv("fullset_WC.csv", stringsAsFactors = FALSE)
...
plotmeans(f$assault_rate ~ f$Year,
          xlab="Year",
          ylab="Assaults per 10,000 people",
          main="Means and CI95 of Assault Rate, by Year")
plotmeans(f$rape_rate ~ f$Year,
          xlab="Year",
          ylab="Rapes per 10,000 people",
          main="Means and CI95 of Rape Rate, by Year")
plotmeans(f$murder_rate ~ f$Year,
          xlab="Year",
          ylab="Murders per 10,000 people",
          main="Means and CI95 of Muder Rate, by Year")
plotmeans(f$rob_rate ~ f$Year,
          xlab="Year",
          ylab="Robberies per 10,000 people",
          main="Means and CI95 of Robbery Rate, by Year")
plotmeans(f$Gear_Spend/1000000 ~ f$Year,
          xlab="Year",
          ylab="Military Gear Value ($M)",
...
summary(bm.44)
bm.8 <- plm(log(Gear_Spend) ~ murder_rate + 
              rape_rate +
              rob_rate +
              assault_rate +
              burg_rate +
...
bm.8 <- coeftest(bm.8, vcov=vcovHC)
# bm.444 <- coeftest(bm.444, vcov=vcovHC)
stargazer(bm.8, title="Aggregate Crime Rates; Race, 2-Way FEs, CSEs")
