...
sales.df$BRAND <- as.factor(str_trim(sales.df$BRAND, "both"))
...
sales_bybrand <- sales.mid %>% 
  ...
  mutate(Sales = sum(SALES),
         Cost = sum(COST),
         Revenue = sum(REVENUE))
sales_bybrand <- distinct(select(sales_bybrand, Date, Brand = BRAND, Sales, Cost, Revenue)) 
...
sales_bybrand$Revenue <- ifelse(sales_bybrand$Date >= billed_day, sales_bybrand$Revenue*0.967, sales_bybrand$Revenue)  

# Make GM calcs
sales$GM <- sales$Revenue-sales$Cost
sales_bybrand$GM <- sales_bybrand$Revenue-sales_bybrand$Cost
sales$GMPerc <- sales$GM/sales$Revenue
sales_bybrand$GMPerc <- sales_bybrand$GM/sales_bybrand$Revenue
...
ics_bybrand <- left_join(imps_clicks_bybrand, sales_bybrand, by = c("Date" = "Date", "Brand" = "Brand"))
...
ics_bybrand_test <- ics_bybrand %>% 
      ...
      mutate(
        ...
        Cost_7 = SMA(Cost, 7),
        Cost_30 = SMA(Cost, 30),
        Revenue_7 = SMA(Revenue, 7),
        Revenue_30 = SMA(Revenue, 30),
        GM_7 = SMA(GM, 7),
        GM_30 = SMA(GM, 30),
        GMPerc_7 = SMA(GM/Revenue, 7),
        GMPerc_30 = SMA(GM/Revenue, 30),
...
# filter to date
ics_bybrand_test <- filter(ics_bybrand_test, Date >= "2015-11-10")
...
#######       by brand graphs ####
plot_sales_TW <- ggplot(subset(ics_bybrand_test, Brand == "Total Wireless")) + 
...
plot_sales_ST <- ggplot(subset(ics_bybrand_test, Brand == "Straight Talk")) + 
  geom_line(aes(Date, Sales, color = "Total Sales"))
(plot_sales_ST <- plot_sales_ST + 
  geom_line(aes(Date, Sales_7, color = "7-SMA"), lwd = 1.1) + 
  geom_line(aes(Date, Sales_30, color = "30-SMA"), lwd = 1.1) +
  theme_calc() + scale_color_manual(values = c("blue","red","grey")) +
  guides(color=guide_legend(title="")) + ggtitle("Straight Talk")) + coord_cartesian(ylim = c(0,2800))

plot_sales_NT <- ggplot(subset(ics_bybrand_test, Brand == "Net10")) + 
  geom_line(aes(Date, Sales, color = "Total Sales"))
(plot_sales_NT <- plot_sales_NT + 
  geom_line(aes(Date, Sales_7, color = "7-SMA"), lwd = 1.1) + 
  geom_line(aes(Date, Sales_30, color = "30-SMA"), lwd = 1.1) +
  theme_calc() + scale_color_manual(values = c("blue","red","grey")) +
  guides(color=guide_legend(title="")) + ggtitle("Net 10")) + coord_cartesian(ylim = c(0,380))

plot_sales_TF <- ggplot(subset(ics_bybrand_test, Brand == "Tracfone")) + 
  geom_line(aes(Date, Sales, color = "Total Sales"))
(plot_sales_TF <- plot_sales_TF + 
  geom_line(aes(Date, Sales_7, color = "7-SMA"), lwd = 1.1) + 
  geom_line(aes(Date, Sales_30, color = "30-SMA"), lwd = 1.1) +
  theme_calc() + scale_color_manual(values = c("blue","red","grey")) +
  guides(color=guide_legend(title="")) + ggtitle("Tracfone")) + coord_cartesian(ylim = c(0,1750))
