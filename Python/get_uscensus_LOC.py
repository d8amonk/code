...
for s in range(1,72):
    for c in range(1,131,2): #could sample every 5th coutny using range(1,508,5)
...
c_txt = soup.find_all("td", attrs={'headers':'rp9'})[0].text
s_txt = soup.find_all("td", attrs={'headers':'rp9'})[1].text
c_flt = float(str(c_t).split("%")[0])
s_flt = float(str(s_t).split("%")[0])
c_share = round(c_f/100,3)
s_share = round(s_f/100,3)

state_list = soup.find_all("option")
for state in state_list:
    fips_s_id = state['value'].split(".")[0]
    fips_s_name = state.text
    print fips_s_id, fips_s_name, "<- complete!"
